...
const DropdownSeachStyle = styled.div`
	...
	backdrop-filter: blur(
		5px
	); /* You may need vendor prefixes for some browsers */
...
const SearchBarFull = ({
	...
	const handleSearch = (searchProp: string) => {
		...
		socket.current.emit("search", { search: searchProp, limit: 3 });
	};
	...
	return (
		...
		fullScreenDropdown && (
			<DropdownSeachStyle
				className="full-screen-dropdown-search"
				ref={searchBarRef}
			>
				<SearchBarStyle className="full-screen-search-bar w-full">
					...
					<div
						className="search-bar-input flex flex-row items-center gap-4 w-full 
                        "
					>
				...
				<div className="mt-5 w-full h-full">
					{!searchResults["users"] && !searchResults["channels"] && (
						<div className="search-results-none-message flex flex-col mt-5 items-center w-ful h-full">
							<h1 className="text-2xl font-bold text-black-500 w-9/12 text-center">
								{" "}
					...
					searchResults["channels"].length === 0 ? (
						<div className="search-results-none-message flex flex-col mt-5 items-center w-ful h-full">
							<h1 className="text-2xl font-bold text-black-500 w-9/12 text-center">
								{" "}
								Nope, can't find it. Did you check under the couch.{" "}
							</h1>
					...
					<div className="search-results">
						{searchResults["users"] && searchResults["users"].length > 0 ? (
							<div className="search-results-header flex flex-row items-center w-full justify-between">
								<h1 className="text-base font-bold text-zinc-800 w-9/12 text-left">
									People
								</h1>
						...
						{searchResults["users"] &&
							searchResults["users"].map((user: any, index: number) => {
								return (
									<div
										className="search-result flex flex-row jusotfy-between items-center gap-4 py-0.5 w-full rounded-lg"
										key={index}
									>
										<div className="search-result-avatar w-full flex flex-row rounded-lg transition duration-200 ease-in-out hover:bg-[rgba(0,0,0,0.1)] cursor-pointer py-2">
											...
											{user.id === Cookies.get("id") ? null : (
												<div className="chat-button flex justify-center items-center mr-1">
													<a
														className="chat-button drop-shadow-2xl rounded-full p-2 hover:bg-[#27272a] hover:text-white"
														onClick={(e) => {
															e.preventDefault();
															e.stopPropagation();
															handleTempChat(user);
						...
						{searchResults["channels"] &&
							searchResults["channels"].map((channel: any, index: number) => {
								return (
									<div
										className="search-result flex flex-row jusotfy-between items-center gap-4 py-0.5 w-full rounded-lg"
										key={index}
									>
										<div className="search-result-avatar w-full rounded-lg transition duration-200 ease-in-out hover:bg-[rgba(0,0,0,0.1)] cursor-pointer py-2">
											<Link
												to={`/user/${channel.id}`}
												className="flex flex-row flex-start items-center gap-4 w-full"
											>
