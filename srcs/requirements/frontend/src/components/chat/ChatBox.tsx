...
const ChatBox = ({
	...
	const updateSeenStatus = (messages: singleMessage[]) => {
		messages.forEach((message: any) => {
			if (message.seen === false && message.sender_id !== Cookies.get("id")) {
				axios
					.put(
						`http://localhost:3000/api/v1/chatrooms/private/${message.chatRoom_id}/message/${message.id}`,
						{
							seen: true,
						}
					)
	...
	const getMessages = async (currentChat: any) => {
		...
		let responseMessages = await axios.get(
			`http://localhost:3000/api/v1/chatrooms/private/${currentChat.chatRoomid}/messages?limit=${limit}&offset=${offset}`
		);
